...
public:
    ...
    static BigInt bi_eegcd(const BigInt& a, const BigInt& b, BigInt& x, BigInt& y)
    {
        if (a == 0)
        {
            x = 0; y = 1;
    ...
    BigInt powerMod(const BigInt& power, const BigInt& mod) const ;
    ...
    int getBILength() const { return m_lenght; }
    ...
    friend BigInt operator % (const BigInt& left, const BigInt& right)
    {
        return left.bi_mod(right).bi_mod(right);
    }
